...
const calculation = (state = initialState, action) => {
    switch (action.type) {
        ...
        case "DELETE": {
        return { ...state, calculation: state.calculation.length >= 2 ? state.calculation.slice(0, -1) : 0,lastCalculations: state.lastCalculations}
        }
        case "EQUAL": {
          try {
            ...
            return { ...state, calculation:'ERR', lastCalculations: [...state.lastCalculations , {calculation:state.calculation,answer:'ERR'}] }
          }
